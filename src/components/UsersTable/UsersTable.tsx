...
import { fetchUsers, selectUsers, removeUser, loadingUsers, loadingRemoveUser } from "../../redux/user/userSlice";
import { fetchBlogPosts, selectBlogPosts, removeBlogPost } from "../../redux/blog/blogPostSlice";
...
export const UsersTable = () => {
  ...
  const [currentPostAuthor, setCurrentPostAuthor] = useState<User | undefined>();
  ...
  useEffect(() => {
    const expandedPosts = blogPosts.filter((post) => post.userId === expandedUserId);
  ...
  const subsetOfUsers: User[] = filteredItems.slice(firstUserIndex, lastUserIndex);
    ...
    <div className="container">
      ...
      <div className="search-holder">
        <div className="search-filter">
          ...
          <input type="search" id="search-filter" onChange={(e) => handleSearchFilter(e.target.value)} value={filterQuery} />
        </div>
      </div>
          ...
          <tbody>
            {subsetOfUsers.map((user) => (
              <React.Fragment key={user.id}>
                  <tr className={`table-row ${expandedUserId === user.id ? "expanded": ""}`}>
                      <td><button aria-expanded="false" className="expand-row" onClick={() => handleExpandRow(user.id)}></button></td>
                      ...
                      <td><button type="button" onClick={() => openCreatePostForm(user)}>Create New Post</button></td>
                      <td><button type="button" className="delete-btn" onClick={() => handleDeleteButton(user.id)}>Delete</button></td>
                  </tr>
                              ...
                              <div className="blog-list-holder">
                                <ul className="blog-list">
                                    {expandedUserPosts.map((post) => (
                                        <li key={post.id}>
                                            <span className="post-title"><Link to={'/blog-post/' + post.id}>{post.title.substring(0, 60)}...</Link></span>
                                            <span className="post-date">{new Date(post.datePosted).toLocaleDateString()}</span>
                                            <div className="button-holder">
                                              <Link to={'/blog-post/' + post.id} className="button-link">Read more</Link>
                                            </div>
                                            <div className="button-holder">
                                              <button type="button" className="delete-btn" onClick={() => handleDeletePostButton(post.id)}>Delete Post</button>
                                            </div>
                                        </li>
                                    ))}
      ...
      <Pagination currentPage={currentPage} setCurrentPage={setCurrentPage} usersCount={filteredItems.length} usersPerPage={usersPerPage} />
      {formVisible && (
          <AddPostForm postAuthor={currentPostAuthor} setFormVisible={setFormVisible}/>
      )}
      {loadingFetchUsers && (
        <div className="loading-overlay">
          <span className="loader"></span>
